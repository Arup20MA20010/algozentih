...
void solve(set<char> vowels, int t, int test)
{
    string ans = "";
    string kingdom = "";
    cin >> kingdom;
    if (vowels.find(kingdom[kingdom.length() - 1]) == vowels.end())
    {
        if (kingdom[kingdom.length() - 1] == 'y')
        {
            ans = "nobody";
        }
        else
    ...
    cout << "Case #" << test - t << ": " << kingdom << " is ruled by " << ans << "." << nline;
}
int main()
{
