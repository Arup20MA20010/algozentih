...
int rec(int level, int row, int col)
{
    ...
    if (row == 3 || col == 3 || row <= 0 || col <= 0)
        return 0;
    return rec(level - 1, row, col + 1) + rec(level - 1, row, col - 1) + rec(level - 1, row + 1, col) + rec(level - 1, row - 1, col) + rec(level - 1, row - 1, col - 1) + rec(level - 1, row + 1, col - 1) + rec(level - 1, row - 1, col + 1) + rec(level - 1, row + 1, col + 1);
}
void solve()
{
    cin >> n;
