...
{
    // base case
    if (level1 == s1.length() || level2 == s2.length())
    {
        return dp[level1][level2] = 0;
    }
    // cache check
    ...
    // transition and chache
    if (s1[level1] == s2[level2])
    {
        return dp[level1][level2] = 1 + rec(level1 + 1, level2 + 1);
    }
    return dp[level1][level2] = max(rec(level1 + 1, level2), rec(level1, level2 + 1));
}
void solve()
{
