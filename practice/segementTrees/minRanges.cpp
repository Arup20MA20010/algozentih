...
void build(int index, int l, int r)
{
    ...
    if (t[2 * index].ff == t[2 * index + 1].ff)
    {
        t[index].ss = t[2 * index].ss + t[2 * index + 1].ss;
    }
    else
    {
...
pair<int, int> query(int index, int l, int r, int lq, int rq)
{
    if (l > rq || r < lq)
    {
        return {INT_MAX, 0};
    ...
    int mid = (l + r) / 2;
    pair<int, int> p1 = query(2 * index, l, mid, lq, rq);
    pair<int, int> p2 = query(2 * index + 1, mid + 1, r, lq, rq);
    if (p1.ff == p2.ff)
    {
