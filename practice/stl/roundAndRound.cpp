...
pair<pair<int, int>, int> move_custom(char ch, pair<int, int> coordinates, int direction)
{
    if (ch == 'W')
    {
        if (direction == 0)
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        pair<pair<int, int>, int> newPos = move_custom(s[i], coordinates, direction);
        coordinates = newPos.ff;
        direction = newPos.ss;
    }
    if (coordinates == make_pair(0, 0) || direction != 0)
    {
        cout << "YES" << nline;
    }
    else
