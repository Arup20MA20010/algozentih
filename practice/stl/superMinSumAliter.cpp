...
ll n;
ll arr[100100];
ll nextSmaller[100100];
ll prevSmaller[100100];
void solve()
{
    cin >> n;
    for (int i = 0; i < n; i++)
    ...
    for (int i = n - 1; i >= 0; i--)
    {
        while (nextSmaller[i] < n && arr[i] <= arr[nextSmaller[i]])
        {
            nextSmaller[i] = nextSmaller[nextSmaller[i]];
        }
    }
    for (int i = 0; i < n; i++)
    {
        while (prevSmaller[i] >= 0 && arr[i] <= arr[prevSmaller[i]])
        {
    ...
    for (int i = 0; i < n; i++)
    {
        sum += (i - prevSmaller[i]) * (nextSmaller[i] - i) * arr[i];
    }
    cout << sum << nline;
}
int main()
